...
! Modified for general RDM (noncanonical) by Quan Phung, Leuven, July 2017
...
! Reset chemps2_restart = .false. if not checkpoint files
     ...
     &   (IRST.EQ.0 .AND. (chemps2_restart.EQV..FALSE.))) THEN
...
! Quan: overwrite CheMPS2_xxxorb_MPSX.h5 to CheMPS2_MPSX.h5
         if (
     &   (IFINAL.EQ.2 .AND. Do3RDM) .OR.
     &   (IFINAL.EQ.2 .AND. iOrbTyp.EQ.2)) THEN
