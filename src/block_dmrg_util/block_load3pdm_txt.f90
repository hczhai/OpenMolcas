...
subroutine block_load3pdm_txt( NAC, PT, CHEMROOT, TRANS, doG3 )

  IMPLICIT NONE
  INTEGER, INTENT(IN) :: NAC, CHEMROOT
  REAL*8, INTENT(OUT) :: PT( NAC, NAC, NAC, NAC, NAC, NAC )
  LOGICAL, INTENT(IN) :: TRANS, doG3
  REAL*8 :: PTtemp
  INTEGER :: nac6
...
!  PT = 0.0d0
  ...
  if (doG3) then
    ...
    if (trans) then
      ...
      file_3rdm="./node0/fock_fourpdm."//trim(adjustl(rootindex))//"."//trim(adjustl(rootindex))//".txt"
    endif
  endif
